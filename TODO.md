...
- ~~Multilingual support — Afrikaans first, then Zulu/Xhosa~~ — SHIPPED 2026-05-23 with **Afrikaans AND isiZulu** simultaneously (Kenny: "then isiZulu please"). Migration 018 adds `children.preferred_language` with CHECK ('en','af','zu'). New `src/lib/languages.ts` is single source of truth — `buildLanguageDirective(code, name)` returns a language-specific system-prompt prefix that handles bilingual code-switching, age-appropriate register, and the English-for-technical-terms allowance for isiZulu. Wired into `/api/chat`, `/api/voice-llm`, and `/api/voice-session` (which also passes `languageCode` back so VoiceTalk applies it as EL's `overrides.agent.language` for STT). AddChildForm has a new "Echo speaks" picker (English / Afrikaans / isiZulu). Dashboard children card shows the non-English language as a small chip next to the tutor name. Adding Xhosa later is one edit per file (lib + migration + UI option).
  - **Untested in production yet:** voice quality in Afrikaans/isiZulu depends on EL's voice support. The default Echo voice may not handle isiZulu naturally — may need a per-language `ELEVENLABS_VOICE_ID_AF` / `_ZU` env-var with appropriate native voices. Test on the Packard with each language and add per-language voice overrides if needed.
